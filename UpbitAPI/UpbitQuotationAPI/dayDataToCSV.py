...
#조회할 일봉 데이터 기간의 가장 최근 연/월(31일 23:59:00 이 기준)
startYear=2021
startMonth="12"

#조회할 일봉 데이터 기간의 가장 오래된 연/월(01일 00:00:00 이 기준)
endYear=2021
endMonth="03"
...
url = "https://api.upbit.com/v1/candles/days?market="+marketCode+"&to="+str(startYear)+"-"+startMonth+"-31 00:00:00&count=200"
...
while True:
    ...
    sleep(0.11)
    url = "https://api.upbit.com/v1/candles/days?market="+marketCode+"&to="+str(year)+"-"+newMonth+"-"+newDay+" 00:00:00&count=200"
    headers = {"Accept": "application/json"}
    response = requests.request("GET", url, headers=headers)
    responseTxt = response.text
...
df.to_csv("DayCandle_"+marketCode+"_"+str(startYear)+"."+startMonth+"_"+str(endYear)+"."+endMonth+".csv", quoting=1)
